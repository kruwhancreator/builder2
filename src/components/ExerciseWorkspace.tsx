...
export default function ExerciseWorkspace({ chapter, chapterData }: ExerciseWorkspaceProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-2 sm:px-4 py-6">
      {/* 1. Header Card (Matching sentence_builder_unit1_answers.html) */}
      <div className="bg-gradient-to-br from-[#1e3a8a] to-[#2563eb] text-white rounded-2xl p-6 sm:p-8 mb-6 shadow-md relative overflow-hidden">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold font-heading mb-1.5">
          เฉลยแบบฝึกหัด Unit {chapterData.chapter}: {chapterData.subtitle || 'ฉันกำลัง...'}
        </h1>
      ...
      <div className="bg-white border-l-5 border-[#2563eb] rounded-xl p-4 sm:p-5 shadow-xs mb-8 flex items-start sm:items-center gap-4">
        <div className="w-13 h-13 rounded-full bg-gradient-to-br from-sky-100 to-sky-200 text-[#1e3a8a] flex items-center justify-center text-2xl shrink-0 border-2 border-[#2563eb]">
          👩‍🏫
        </div>
      ...
      {ex1 && (
        <div className="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-xs mb-8">
          <div className="border-b-2 border-blue-50 pb-3 mb-6">
            <h2 className="text-xl sm:text-2xl font-bold text-[#1e3a8a] font-heading flex items-center gap-2">
              ✏️ Exercise 1: แต่งประโยคภาษาอังกฤษ
            </h2>
            <div className="bg-[#eff6ff] text-[#1e40af] p-3 rounded-lg text-xs sm:text-sm mt-3 border-l-3 border-[#2563eb]">
              📌 <b>คำแนะนำจากครูหวาน:</b> โปรดใช้คำศัพท์จาก <b>Unit {chapterData.chapter} ในหนังสือ Sentence Builder 2</b> ในการตอบนะคะ ระบบจะตรวจคำตอบแบบเป๊ะๆ (รวมถึงการพิมพ์ตัวพิมพ์ใหญ่ ตัวพิมพ์เล็ก และจุด Full Stop <code className="bg-white px-1 rounded text-blue-700">.</code> ด้านหลังด้วยน้า)
            </div>
          </div>
                      ...
                      {fb.points && fb.points.length > 0 && (
                        <ul className="space-y-1 mt-1.5 pl-1">
                          {fb.points.map((pt, pIdx) => (
                            <li key={pIdx} className="font-semibold text-xs sm:text-sm leading-relaxed">
                              {pt}
                            </li>
                          ))}
                        </ul>
      ...
      {ex2 && (
        <div className="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-xs mb-8">
          <div className="border-b-2 border-blue-50 pb-3 mb-6">
            ...
            <div className="bg-[#eff6ff] text-[#1e40af] p-3 rounded-lg text-xs sm:text-sm mt-3 border-l-3 border-[#2563eb]">
              📌 <b>คำแนะนำจากครูหวาน:</b> ให้เลือกคำจากตารางด้านล่างนี้ในหนังสือ <b>Sentence Builder 2</b> มาเติมในช่องว่างให้สมบูรณ์ ตรวจเช็คการสะกดคำและเครื่องหมายให้ถูกต้องนะคะ
            </div>
          </div>
      ...
      {ex3 && (
        <div className="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-xs mb-8">
          <div className="border-b-2 border-blue-50 pb-3 mb-6">
            <h2 className="text-xl sm:text-2xl font-bold text-[#1e3a8a] font-heading flex items-center gap-2">
              🖼️ Exercise 3: ดูภาพแล้วแต่งประโยคโดยใช้โครงสร้าง Core + Context + Connect
            </h2>
            <div className="bg-[#eff6ff] text-[#1e40af] p-3 rounded-lg text-xs sm:text-sm mt-3 border-l-3 border-[#2563eb]">
              🌟 <b>ข้อแนะนำพิเศษจากครูหวาน:</b> แบบฝึกหัดนี้ใช้จินตนาการแต่งประโยคจากภาพได้เลยนะคะ <b>ไม่มีถูกไม่มีผิด!</b> ลองแต่งประโยคตามโครงสร้าง 3 กล่องด้านล่างได้เลยค่ะ
            </div>
          </div>
